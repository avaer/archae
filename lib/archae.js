...
class ArchaeClient extends EventEmitter {
  ...
  releasePlugins(pluginNames) {
    const releasePluginPromises = pluginNames.map(pluginName => this.releasePlugin(pluginName));
  ...
  connect() {
    ...
    connection.on('connect', () => {
      ...
      this.onMessageType('change', (err, result) => {
        if (!err) {
          const {plugin} = result;

          this.emit('change', plugin);
        } else {
          console.warn(err);
        }
      });
